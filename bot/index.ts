...
async function sendDailyReportToChat(chatId: string | number, date?: string) {
  try {
    ...
    const res = await axios.get(
      `${BACKEND_BASE_URL}/reports/daily-text`,
      {
        params: { date: dateStr },
      }
    );
...
async function sendMainMenu(ctx: Context, isManager: boolean | string | undefined) {
  const lines: string[] = [];
...
bot.command("start", async (ctx: Context) => {
  ...
  const isManager =
    MANAGERS_CHAT_ID && String(ctx.chat.id) === String(MANAGERS_CHAT_ID);
  ...
  await ctx.reply(
    "Илтимос, бот сизни таниши учун контактни юборинг.\n" +
...
bot.on("message:contact", async (ctx: Context) => {
  ...
  const isManager =
    MANAGERS_CHAT_ID && ctx.chat && String(ctx.chat.id) === String(MANAGERS_CHAT_ID);
  await sendMainMenu(ctx, isManager);
});

// Employee self-service flows
bot.hears("Опаздываю", async (ctx: Context) => {
  const kb = new InlineKeyboard()
    .text("15 мин", "late_15")
    .text("30 мин", "late_30")
    .text("60 мин", "late_60");
...
bot.hears("Болею", async (ctx: Context) => {
  if (!ctx.chat) return;
  pendingStates.set(ctx.chat.id, { type: "sick" });
  await ctx.reply(
    "Понимаю. Напишите коротко причину / комментарий одним сообщением."
  );
});

bot.hears("Отгул", async (ctx: Context) => {
  if (!ctx.chat) return;
  pendingStates.set(ctx.chat.id, { type: "dayoff" });
  await ctx.reply(
    "Принято. Напишите коротко причину / комментарий одним сообщением."
  );
});

bot.callbackQuery(/late_(\d+)/, async (ctx: Context) => {
  ...
  await ctx.reply(
    `Опоздание на ~${minutes} минут. Укажите, пожалуйста, причину одним сообщением.`
  );
});
...
bot.on("message:text", async (ctx: Context, next: NextFunction) => {
  ...
  try {
    if (state.type === "late") {
      await axios.post(`${BACKEND_BASE_URL}/employee/status/late`, {
        telegramUserId: String(ctx.from.id),
        fullName,
      ...
      await axios.post(`${BACKEND_BASE_URL}/employee/status/sick`, {
        telegramUserId: String(ctx.from.id),
        fullName,
        reason,
      });
      ...
      await axios.post(`${BACKEND_BASE_URL}/employee/status/dayoff`, {
        telegramUserId: String(ctx.from.id),
        fullName,
        reason,
      });
...
bot.command("today", async (ctx: Context) => {
  if (ctx.chat) await sendDailyReportToChat(ctx.chat.id);
});
...
bot.command("dashboard", async (ctx: Context) => {
  ...
  if (replyMarkup) {
    ...
    await ctx.reply(
      "Дашборд в Telegram открывается только по HTTPS. Локально откройте в браузере: " +
